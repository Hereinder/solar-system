...
public:
    // model data
    std::vector<TextureS> textures_loaded;  // stores all the textures loaded so far, optimization to make sure textures
    ...
    Model(std::string const &modelPath, std::string const &shaderPath, bool gamma = false) : gammaCorrection(gamma) {
        loadModel(modelPath);
    ...
    void DrawModel() {
        for (unsigned int i = 0; i < m_Meshes.size(); i++) m_Meshes[i].Draw(m_Shader.get());
    }
...
private:
    ...
    void loadShader(std::string const &path) {
        // read file via ASSIMP
        m_Shader = std::make_unique<Shader>(path);
    }
...
inline unsigned int TextureFromFile(const char *path, const std::string &, bool) {
