...
void TreeObject::LoadMesh() {
    ...
    // m_Indices = new unsigned int[]{0, 1, 2, 2, 3, 0, 5, 6, 7, 7, 4, 5, 8, 9, 10, 10, 11, 8, 13, 14, 15, 15, 12, 13};
    m_Indices =
    ...
    // m_ib = std::unique_ptr<IndexBuffer>(m_Indices, sizeof(m_Indices) / sizeof(unsigned int));
...
void TreeObject::Draw(const glm::mat4&, const glm::mat4&, const glm::vec3&) {
    glm::vec3 m_Translation(300, 300, 0);
    glm::mat4 proj;
    glm::mat4 view;
    ...
    // glm::mat4 rot0 = glm::mat4(1.0f);
    {
        ...
        // model = glm::translate(glm::mat4(1.0f), m_Translation) * glm::rotate(rotY, to_radian(ang[2]), glm::vec3(0, 0,
        // 1));
